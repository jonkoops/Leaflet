...
Depending on the type of event, a map handler can attach event listeners to the `document`, the `window`, or the container of the `LeafletMap` it's attached to.

## Controls

You already know controls - the zoom control in the top left corner, the scale at the bottom left, the layer switcher at the top right. At their core, a `Control` is an HTML Element that is at a static position in the map container.
...
If your custom control consists of more than one HTML element (like `ZoomControl`, which has two buttons), you'll have to create the whole hierarchy of elements and return the topmost container.
